...
void Main()
{
	var program = File.ReadAllLines($"{Path.GetDirectoryName(Util.CurrentQueryPath)}/19-2.txt");
	var registers = new Register(new[] { 1, 0, 0, 0, 0, 0 });
...
public Action<Register> Compile(string instruction)
{
	...
	{
		case "innerloop": return registers => {
			//mulr 5 3 4
			//eqrr 4 1 4
			//addr 4 2 2
			//addi 2 1 2
