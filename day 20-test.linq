...
void Main()
{
	...
	foreach (var regex in regexes)
	{
		...
		var longestShortestDistance = 0;
		Print(rooms);
		foreach (var key in rooms.Keys)
		{
			var path = BuildPath(rooms, (0, 0), (key.X, key.Y));
			longestShortestDistance = Math.Max(longestShortestDistance, BuildPath(rooms, (0, 0), (key.X, key.Y)).Count-1);
		}
		longestShortestDistance.Dump();
	}
}
public class Room
{
